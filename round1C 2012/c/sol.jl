    ...
    for i = 1:N, j = 1:M
        ...
        if A[i] == B[j]
            ...
            if i == 1 && j != 1
                countA = cumA[1, typ] 
                for nj = 1:j-1
                    countB = cumB[j, typ] - cumB[nj, typ]
                    dp[i, j] = max(dp[i, j], min(countA, countB))
                end
            end
            if i != 1 && j == 1
                countB = cumB[1, typ]
                for ni = 1:i-1
                    countA = cumA[i, typ] - cumA[ni, typ] 
                    dp[i, j] = max(dp[i, j], min(countA, countB))
                end
            end

            for ni = 1:i-1, nj = 1:j-1
                countA = cumA[i, typ] - cumA[ni, typ]
                countB = cumB[j, typ] - cumB[nj, typ]
                dp[i, j] = max(dp[i, j], dp[ni, nj] + min(countA, countB))
            end
            dp[i, j] = max(dp[i, j], min(cumA[i, typ], cumB[j, typ]))
        end
    end
    # println(dp)
    dp[N, M]
